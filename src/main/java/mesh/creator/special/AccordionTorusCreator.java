...
public class AccordionTorusCreator implements IMeshCreator {
  ...
  private void createVertices() {
    ...
    for (int i = 0; i < majorSegments; i++) {
      ...
      new RotateZModifier(-Mathf.HALF_PI).modify(circle);
      new RotateYModifier(Mathf.HALF_PI - majorAngle).modify(circle);
  ...
  private void createFaces() {
    for (int i = 0; i < majorSegments; i++)
      for (int j = 0; j < minorSegments; j++) createFaceAt(i, j);
  }
