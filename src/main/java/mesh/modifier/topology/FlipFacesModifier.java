...
/**
 * A modifier responsible for inverting the indices of 3D mesh faces. This is typically used to
 * reverse the winding order of faces, which can impact rendering order, normal direction, or
 * surface orientation.
 *
 * <p>Note: This modifier only flips the indices of the faces. It does not account for ensuring the
 * correct order of vertices relative to surface normals or other geometric properties. Additional
 * adjustments may be needed for proper rendering or normal recalculations.
 *
 * <p>Implements both single-face and collection-based operations to handle individual or multiple
 * faces in a given mesh.
 */
public class FlipFacesModifier implements IMeshModifier, FaceModifier {
  ...
  /**
   * Validates that the provided mesh and a collection of faces are valid and not null.
   *
   * @param mesh The 3D mesh to validate.
   * @param faces The collection of faces to validate.
   * @throws IllegalArgumentException if either the mesh or the faces collection is null.
   */
  ...
  /**
   * Performs in-place index inversion on a single face's indices to change its winding order.
   *
