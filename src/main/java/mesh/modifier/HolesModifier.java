...
/**
 * The {@code HolesModifier} class modifies a 3D mesh by creating holes in its faces. This is
 * achieved by insetting specified faces to a percentage of their original size, effectively leaving
 * holes where the faces used to be.
 *
 * <pre>
 *
...
public class HolesModifier implements IMeshModifier, FaceModifier {
  ...
  /**
   * The percentage of the face's size to retain after creating the hole. A value of 0.0 leaves the
   * face unchanged, while hole consumes the full face area with a value of 1.0.
   */
  private float holePercentage;

  /** Default constructor initializes the modifier with a hole percentage of 0.5 (50%). */
   ...
   *
   * @param holePercentage The percentage of the face's size to retain after creating the hole. Must
  ...
  /**
   * Modifies the given mesh to create holes by insetting it's faces to a specified percentage of
   * their original size.
   *
   * @see #HolesModifier()
  ...
  /**
   * Modifies the given mesh to create holes by insetting the specified faces to a specified
  ...
  /**
   * Modifies the given mesh to create holes by insetting the specified face to a specified
  ...
  /**
   * Creates an instance of {@code ExtrudeModifier} configured to create holes. The modifier will
   * scale faces based on the specified hole percentage, with no extrusion applied, and will remove
   * the original faces.
   *
  ...
  /**
   * Determines whether the modification can exit early based on the current mesh state and hole
   * percentage.
   *
   * @param mesh The mesh to evaluate for early exit.
   * @return {@code true} if the modification can exit early (e.g., the mesh has no faces or the
  ...
  /**
   * Gets the current percentage of the face's size retained after creating the hole.
   *
   ...
   *
   * @param holePercentage The new hole percentage to set. Must be between 0.0 and 1.0, inclusive.
   * @throws IllegalArgumentException if the percentage is outside the valid range.
   */
  public void setHolePercentage(float holePercentage) {
    if (holePercentage < 0 || holePercentage > 1) {
      throw new IllegalArgumentException("Hole percentage must be between 0 and 1 inclusive.");
    }
