...
/**
 * The {@code WaveModifier} class applies a wave-like deformation to a 3D mesh, simulating the
 * appearance of sinusoidal waves across the surface of the mesh.The wave modifier is particularly
 * useful for simulating effects like water surfaces, undulating terrain, or other wave-like
 * phenomena in computer graphics. This modifier operates by displacing the vertices of a given mesh
...
public class WaveModifier implements IMeshModifier {
  ...
  /**
   * Constructs a new {@link WaveModifier} with default values for the wave effect parameters. The
   * default values are:
   *
   * <pre>
   * Amplitude: 1
   ...
   * </pre>
   *
   * These default settings generate a basic wave along the X-axis. Users can modify these
   * properties using the corresponding setter methods after instantiation.
   */
  public WaveModifier() {
    setAmplitude(1);
    setWavelength(1);
  ...
  /**
   * Applies the wave effect to the provided mesh by modifying its vertices by using the wave
   * function defined in the {@link WaveModifier}.
   *
   * @param mesh The {@link Mesh3D} instance to be modified. This mesh must not be null.
   * @return The modified {@link Mesh3D} with the wave effect applied to its vertices.
  ...
  /**
   * Applies the wave deformation to all vertices of the mesh. This method does not return a value;
  ...
  /**
   * Applies a wave deformation to a single vertex based on a sinusoidal wave equation.
   *
   * @param vertex The 3D vertex to which the wave deformation is applied. The vertex's position is
   ...
   *
   * @return The wavelength, representing the distance between consecutive wave peaks.
   */
   ...
   *
   * @param wavelength The distance between consecutive wave peaks. Must be positive.
   * @throws IllegalArgumentException if the wavelength is less than or equal to zero.
   */
   ...
   *
   * @return A {@link Vector3f} representing the unit vector of the wave's direction.
   */
  ...
  /**
   * Sets the direction of the wave. The direction vector is normalized automatically.
   *
   * @param direction A {@link Vector3f} representing the direction of wave propagation. Must not be
