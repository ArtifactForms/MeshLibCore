...
/**
 * CinematicBlackBarsRenderer is responsible for rendering cinematic black bars
 * on the top and bottom of the screen. It supports smooth fade-in and fade-out
 * animations for cinematic effects during transitions.
 * <p>
 * This component renders black bars with configurable speed and handles dynamic
 * animations like appearing or disappearing smoothly over time. The fade
 * effects are controlled by the speed of transition and a constant bar size (in
 * pixels).
 * </p>
 */
public class CinematicBlackBarsRenderer extends AbstractComponent
	...
	/**
	 * Default speed at which the fade-in and fade-out effects occur (in pixels
	 * per second).
	 */
	 ...
	 * <p>
	 * This sets the default fade speed and target bar height to predefined
	 * constants.
	 * </p>
	 */
	 ...
	 * 
	 * @param fadeSpeed       Speed at which fade animations will play (in
	 *                        pixels/second).
	 * @param targetBarHeight The target height of the cinematic bars at full
	 *                        fade-in.
	 */
	 ...
	 * <p>
	 * This method advances the fade-in and fade-out animations by incrementally
	 * updating the `currentSize` value based on elapsed time and speed. Stops
	 * animations when they reach their final states.
	 * </p>
	 * 
	...
	/**
	 * Handles the fade-in animation by incrementally increasing the size of the
	 * black bars over time until the target bar height is reached.
	 * 
	 * @param tpf Time per frame (time elapsed since the last frame in seconds).
	 */
	...
	/**
	 * Handles the fade-out animation by decrementally decreasing the size of the
	 * black bars over time until their visual size is zero.
	 * 
	 * @param tpf Time per frame (time elapsed since the last frame in seconds).
	 */
	 ...
	 * 
	 * @return {@code true} if either fading in or fading out is active,
	 ...
	 * <p>
	 * This stops any ongoing fade-out animation and sets the internal state to
	 * begin the fade-in effect from the current position.
	 * </p>
	 */
	public void fadeIn() {
		if (fadingIn)
	 ...
	 * <p>
	 * This stops any ongoing fade-in animation and sets the internal state to
	 * begin the fade-out effect from the current position.
	 * </p>
	 */
	public void fadeOut() {
		if (fadingOut)
	 ...
	 * <p>
	 * The rendering uses the current interpolated size value (`currentSize`) to
	 * draw smooth black bars transitioning in and out. This rendering respects
	 * the screen's current dimensions and is dynamically updated during
	 * animation.
	 * </p>
	 * 
	...
	private void renderBottomBar(Graphics g) {
		g.fillRect(0, g.getHeight() - (int) currentSize, g.getWidth(),
		    (int) currentSize);
	}
