...
/**
 * The {@code ExtrudeModifier} class implements the functionality of extruding faces of a 3D mesh by
 * a given amount and scaling factor. It allows faces to be extruded outward, inward, and optionally
 * removed from the mesh after extrusion. This class implements {@code IMeshModifier} and {@code
 * FaceModifier} interfaces.
 *
 * <pre>
 * Key features:
 *
...
public class ExtrudeModifier implements IMeshModifier, FaceModifier {
  ...
  /**
   * The default scaling factor applied during extrusion. Defaults to 1.0, which means no scaling.
   */
  private static final float DEFAULT_SCALE = 1.0f;

  /**
   * The default extrusion amount applied to faces. Defaults to 0.0, meaning extrusion with a
   * dinstance of zero.
   */
  private static final float DEFAULT_AMOUNT = 0.0f;

  /**
   * Flag indicating whether the original faces should be removed after extrusion. If true, the
   * original faces are removed; if false, they remain in the mesh.
   */
   ...
   * of the faces, while values greater or less than 1.0 scale the extruded faces proportionally.
   */
  private float scale;

  /**
   * The distance to extrude faces by. Positive values extrude outward along the normal of the face,
   * while negative values extrude inward.
   */
  private float amount;

  /**
   * Default constructor initializing with default scale and amount values. Scale = 1.0, Amount =
   * 0.0.
   */
  ...
  /**
   * Constructor initializing the ExtrudeModifier with a specified scale and amount.
   *
   * @param scale the scaling factor to apply to the extruded geometry (must be >= 0).
   * @param amount the distance to extrude faces by. A positive value extrudes outward; a negative
   *     value extrudes inward.
   */
  ...
  /**
   * Modifies the entire 3D mesh by extruding all faces in the mesh by the current scale and amount.
   *
  ...
  public Mesh3D modify(Mesh3D mesh, Face3D face) {
    ...
    if (removeFaces) mesh.removeFace(face);
    return mesh;
  }
  ...
  private void validateScale(float scale) {
    if (scale < 0) throw new IllegalArgumentException("Scale must be greater than or equal to 0.");
  }
  ...
  private void extrudeFace(Mesh3D mesh, Face3D face) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      mesh.add(vertex);
      mesh.addFace(
          face.indices[i], face.indices[(i + 1) % n], nextIndex + ((i + 1) % n), nextIndex + i);
    }
  ...
  /**
   * Updates the indices of the given face after extrusion to point to the newly created vertices in
   * the mesh's vertex list. This is necessary because new vertices are added during the extrusion
   * process, and their indices must be correctly reflected in the face indices to maintain the
   * mesh's structural integrity.
   *
   * @param face the {@link Face3D} object whose indices need to be updated.
   * @param nextIndex the starting index of the newly added vertices in the mesh's vertex list.
   */
   ...
   *
   * @param amount the distance to extrude. Positive values extrude outward, negative values extrude
   *     inward.
   */
