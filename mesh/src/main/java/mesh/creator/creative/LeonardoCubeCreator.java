...
public class LeonardoCubeCreator implements IMeshCreator {
  ...
  private void createTopConnector() {
    ...
    new TranslateModifier(0, -outerRadius + connectorRadius, 0).modify(top);
    mesh.append(top);
  }

  private void createBottomConnector() {
    Mesh3D bottom = createConnector(true, false);
    new TranslateModifier(0, outerRadius - connectorRadius, 0).modify(bottom);
    mesh.append(bottom);
  }
  ...
  private void createLeftConnector() {
    ...
    new TranslateModifier(-outerRadius + connectorRadius, 0, 0).modify(left);
    mesh.append(left);
  }
  ...
  private void createRightConnector() {
    ...
    new TranslateModifier(outerRadius - connectorRadius, 0, 0).modify(right);
    mesh.append(right);
  }
  ...
  private void extrude(FaceSelection selection, boolean extrudeEnds) {
    ...
    for (Face3D face : selection.getFaces()) {
      ...
      if (extrudeEnds) Mesh3DUtil.extrudeFace(mesh, face, 1, (2 * connectorRadius));
    }
  }
