...
public class CubeJointLatticeCubeCreator implements IMeshCreator {
  ...
  private void createJoints() {
    for (int z = 0; z < cubes[0][0].length; z++)
      for (int y = 0; y < cubes.length; y++)
        for (int x = 0; x < cubes[0].length; x++) createJointAt(x, y, z);
  }
  ...
  private void connectJoints() {
    for (int z = 0; z < cubes[0][0].length; z++)
      for (int y = 0; y < cubes.length; y++)
        for (int x = 0; x < cubes[0].length; x++) connectJointAt(x, y, z);
  }

  private void connectJointAt(int x, int y, int z) {
    if ((x + 1) < cubes[0].length) connectRightLeft(x, y, z);
    if ((y + 1) < cubes.length) connectBottomTop(x, y, z);
    if ((z + 1) < cubes[0][0].length) connectFrontBack(x, y, z);
  }
