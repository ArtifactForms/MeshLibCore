...
public class TruncatedCubeCreator implements IMeshCreator {
  ...
  private float b = Mathf.sqrt(2.0f) - 1.0f;
  ...
  private void createFaces(Mesh3D mesh) {
    createTriangularFaces(mesh);
    createOctagonalFaces(mesh);
  }

  private void createTriangularFaces(Mesh3D mesh) {
    mesh.addFace(0, 16, 8);
    mesh.addFace(1, 9, 17);
  ...
  private void createOctagonalFaces(Mesh3D mesh) {
