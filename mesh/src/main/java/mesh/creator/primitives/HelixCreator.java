  ...
  private void createVertices() {
    ...
    for (int n = 0; n < turns; n++) {
      for (int j = 0; j < majorSegments; j++) {
        Vector3f v0 =
            new Vector3f(
                majorRadius * Mathf.cos(majorAngle), y0, majorRadius * Mathf.sin(majorAngle));
        for (int i = 0; i < minorSegments; i++) {
          Vector3f v1 =
              new Vector3f(
                  minorRadius * Mathf.cos(minorAngle), minorRadius * Mathf.sin(minorAngle), 0);
          // Rotate
  ...
  private void createFaces() {
    ...
    for (int j = 0; j < majorSegments * turns - 1; j++) {
      for (int i = 0; i < minorSegments; i++) {
        ...
        mesh.addFace(index0, index1, index3, index2);
      }
    }
  }
