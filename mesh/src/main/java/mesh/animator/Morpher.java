...
public class Morpher extends AbstractAnimator {
  ...
  public void onUpdate(float tpf) {
    for (int i = 0; i < source.getVertexCount(); i++) {
      ...
      velocity =
          this.target.getVertexAt(i).subtract(this.mesh.getVertexAt(i)).normalize().mult(speed);

      tmp.addLocal(velocity);

      if (position.getX() <= target.getX() && tmp.getX() >= target.getX()) tmp.setX(target.getX());

      if (position.getX() >= target.getX() && tmp.getX() <= target.getX()) tmp.setX(target.getX());

      if (position.getY() <= target.getY() && tmp.getY() >= target.getY()) tmp.setY(target.getY());

      if (position.getY() >= target.getY() && tmp.getY() <= target.getY()) tmp.setY(target.getY());

      if (position.getZ() <= target.getZ() && tmp.getZ() >= target.getZ()) tmp.setZ(target.getZ());
