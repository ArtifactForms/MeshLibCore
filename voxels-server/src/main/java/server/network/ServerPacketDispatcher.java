...
import server.usecases.blockpick.BlockPickHandler;
import server.usecases.blockplace.BlockPlace;
import server.usecases.blockplace.BlockPlaceHandler;
import server.usecases.chat.ChatMessageHandler;
import server.usecases.itemdrop.PlayerDropItemHandler;
import server.usecases.join.PlayerJoinHandler;
...
public class ServerPacketDispatcher {
  ...
  public ServerPacketDispatcher(ServerConnection c, UseCaseRegistry reg, GatewayContext context) {
    ...
    register(BlockPickPacket.class, new BlockPickHandler(c, context)::handle);

    register(PlayerDropItemPacket.class, new PlayerDropItemHandler(c, context)::handle);
