...
import voxels.Voxel;
import voxels.VoxelSphereCreator;

public class SphereCommand extends AbstractCommand {

  private final PlayerGateway players;

  private final WorldEditConfig config;

  public SphereCommand(PlayerGateway players, WorldEditConfig config) {
  ...
  public void execute(CommandContext ctx) {
    ...
    if (radius > config.getMaxSphereRadius()) {
      ctx.reply("Max sphere radius: " + config.getMaxSphereRadius());
      return;
    }
    ...
    VoxelSphereCreator creator = new VoxelSphereCreator(center, radius);
    ...
    ctx.reply("Sphere created with " + voxels.size() + " blocks.");
  }
