	...
	public void generate(ChunkData chunk) {
		...
		for (int x = 0; x < ChunkData.WIDTH; x++) {
			for (int z = 0; z < ChunkData.DEPTH; z++) {
				...
				float terrain = generateOctaveNoise(terrainNoise, wx, wz, terrainScale, octaves, persistence,
						lacunarity);
...
//    createTrees(chunk);
	...
	private BiomeType getBiomeAt(float wx, float wz) {
		...
		if (temp > 0.7 && moisture < 0.3f)
			return BiomeType.DESERT;
		if (temp > 0.6 && moisture > 0.6f)
	...
	private BlockType getBlockType(int x, int y, int z, int heightValue, BiomeType biome, ChunkData chunk) {
		...
		if (y > 140)
			return Blocks.SNOW;
		if (y > 110) {
			return terrainNoise.noise(wx * 0.05f, wz * 0.05f) < 0.7f ? Blocks.STONE : Blocks.COBBLE_STONE;
		}

		// Layers: Grass, Dirt, Stone
		if (y == heightValue)
			return getTopLayer(biome);
		if (y >= heightValue - 3)
			return Blocks.DIRT;
		return Blocks.STONE;
	}
	...
	private void createTrees(ChunkData chunk) {
		...
		for (int x = 0; x < ChunkData.WIDTH; x++) {
			for (int z = 0; z < ChunkData.DEPTH; z++) {
				...
				if (!(topBlock == Blocks.GRASS_BLOCK || topBlock == Blocks.SAND))
					continue;
				...
				if (finalChance > 0.65f && isAreaClear(treeMap, x, z, minDistance)) {
					...
					generateLeafCluster(chunk, x, heightValue + treeHeight, z, biome == BiomeType.FOREST ? 3 : 2);
	...
	private float generateOctaveNoise(PerlinNoise noise, float x, float z, float baseScale, int octaves,
