...
public class BasicWorldGenerator2 implements WorldGenerator {
  ...
  public BasicWorldGenerator2(long seed) {
    ...
    // 2. Den Generator erstellen, der dieses Design nutzt (implementiert
    // StructureGenerator)
  ...
  // private void generateLeafCluster(ChunkData chunk, int x, int baseHeight, int
  // z, int radius) {
  ...
  /**
   * Populates the top of the chunk with grass blocks based on deterministic noise. This ensures
   * grass patterns stay the same every time the world is generated. * @param chunk The chunk data
   * to modify.
   */
  /**
   * Thinned out grass generation using deterministic dither noise. Adjusted for low density and
   * more organic gaps.
   *
   * @param chunk The chunk data to modify.
   */
  private void createGrass(ChunkData chunk) {
    Vector3f chunkPos = getPosition(chunk);
    ...
    for (int x = 0; x < ChunkData.WIDTH; x++) {
      for (int z = 0; z < ChunkData.DEPTH; z++) {
        ...
        // 1. Basic check: Must be on a grass block and in air
        ...
        // 2. Large scale noise (Wiesen-Zonen)
        // Still 0.45, but you can lower it to 0.35 for even smaller patches
        double areaNoise = treeNoise.noise(wx * 0.08f, wz * 0.08f);

        // 3. Strong Dithering (Ausdünnung)
        // Increased threshold to 0.5 or 0.6 for significant gaps
        double ditherNoise = treeNoise.noise(wx * 1.5f, wz * 1.5f);

        // Logic:
        // areaNoise < 0.40: Makes the patches themselves slightly smaller
        // ditherNoise > 0.5: Removes about 75% of the grass within the patch
        if (areaNoise < 0.40 && ditherNoise > 0.5) {
