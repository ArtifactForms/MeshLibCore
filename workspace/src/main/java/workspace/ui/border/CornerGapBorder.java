...
/**
 * A custom border implementation that mimics pixel art-style UI borders with missing pixels at each
 * corner. The "size" parameter defines the scale of these gaps, creating a distinctive visual
 * effect similar to pixel art designs.
 *
 * <p>This border has gaps (or empty space) at each corner, allowing it to fit stylistically into a
 * pixel art-themed user interface.
 */
public class CornerGapBorder implements Border {
   ...
   *
   * @param size Defines the scale of the missing gaps at each corner (in pixels). Larger values
   *     increase the size of these gaps, influencing how much of the border is visually omitted at
   *     the corners.
  ...
  /**
   * Renders the border around a specified rectangle area with gaps at each corner, giving it the
   * appearance of a pixel art-style border.
   *
   ...
   * @param width The width of the rectangular area to render the border around.
   * @param height The height of the rectangular area to render the border around.
   */
  @Override
  ...
  /**
   * Draws the visual rectangles forming the North, West, South, and East sides of the border, each
   * leaving a gap at the corners to achieve the pixel art aesthetic.
   */
  ...
  /**
   * Configures the rectangle's frame to know the drawing boundaries for rendering.
   *
  ...
  /**
   * Retrieves the Insets required for this border. Insets define the amount of space the border
   * occupies on all four sides.
   *
