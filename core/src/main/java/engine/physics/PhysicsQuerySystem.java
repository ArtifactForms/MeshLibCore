...
public class PhysicsQuerySystem extends AbstractSceneSystem {
  ...
  public RaycastResult raycast(Vector3f origin, Vector3f direction, float maxDistance) {
    ...
    Float minDistance = Mathf.POSITIVE_INFINITY;
    Ray3f ray = new Ray3f(origin, direction);

    for (ColliderComponent collider : colliders) {
      Float distance = RaycastTests.raycast(ray, collider);
    ...
    RaycastResult result = new RaycastResult(minDistance != null, target, minDistance);
    return result;
  }
