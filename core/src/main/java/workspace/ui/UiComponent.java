...
public class UiComponent implements UiElement {
  ...
  public UiComponent(
      int x, int y, int width, int height, boolean visible, Color foreground, Color background) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
   ...
   *
   * <p>This method checks if the {@code border} is not null before calling the {@code renderBorder}
   * method on the provided graphics context. It uses the element's current width and height to
  ...
  public Insets getInsets() {
    if (border == null) return new Insets();
